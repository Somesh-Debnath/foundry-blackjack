...
contract BlackJack is VRFConsumerBase {
    ...
    // Constructor
    constructor (address vrfCoordinator, address linkToken, bytes32 keyHash_, uint256 fee_) VRFConsumerBase(vrfCoordinator, linkToken) {
    _rngCounter = 1;
    _indexCounter = 1;
    _owner = msg.sender;
    ...
    function GetCard() private returns (uint256 cardValue) {
     ...
     /**
     * @dev GenerateRandomNumber using Chainlink VRF
     */
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal pure override {
        // Handle the randomness as needed
        // In your case, you may want to use it to generate a random card value

        // Ensure randomness is within the desired range (1 to 13)
        uint256 cardValue = (randomness % 13) + 1;
    }
